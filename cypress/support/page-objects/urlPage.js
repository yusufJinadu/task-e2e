...
export default class UrlPage {
    constructor(){
        ...
        this._backgroundColourInput = '.form-group > .color-picker-wrapper > .color-picker-input-wrapper > .color-picker-input'
    ...
    createURLQRCode(url,{colours=null,logo=null} ={}){
        ...
        if(colours){
            cy.get(this.getCustomizationPanes()).contains(this.getText().setColours[lang]).click()
            if(colours.foreColour){
                cy.get(this.getForegroundColourInputs()).eq(0).clear().type(colours.foreColour)
            }
            if(colours.background){
                cy.get(this.getBackgroundColourInput()).clear({force:true}).type(colours.background,{force:true})
            }
        }
        if(logo){
            cy.get(this.getCustomizationPanes()).contains(this.getText().addLogo[lang]).click({force:true})
            if(logo.class){
        ...
        /*
        ideally, I would like to verify the appearance of the generated qr code 
        Using a visual regression tool, but it's paid, so...
        */

        /* I would have also loved to verify that the web version redirects to 
        the correct url, but the plugin I found can only veriy qr codes in HTML element by
        using the ID, and the HTML elemen has no ID*/

...
describe(
	...
	function () {
		...
		it('User can create and verify customized URL QR codes', function () {
			...
			urlPage.createURLQRCode(this.url, {colours:this.colours, logo:{class:'.sprite-logo-instagram-circle'}} )
			//verifyURLQRCode(this.url)
		});
...
export function verifyURLQRCode(url,{downloadsPath = './cypress/downloads',} ={}){
    cy.waitUntil(() => cy.get(urlPage.getQRImage()).invoke('attr','src').then((src) => src.includes('api')))
        cy.get(urlPage.getQRImage()).invoke('attr','src').then((src) => {
            let ImgLink = src.replace(/svg/gi, "png")
            console.log(ImgLink)
            cy.downloadFile(`https:${ImgLink}`,downloadsPath ,'qr-code.png')
        })
        cy.waitUntil(() => cy.task('checkDownloadsExistence',downloadsPath).then((isExisting) =>  isExisting === true)
        ,{timeout:10000}
        )
